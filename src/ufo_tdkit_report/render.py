...
def _credit(ai: bool = False, model: str | None = None) -> str:
    """Bare credit text: tool + version (+ AI model when narrated). No markup."""
    from ufo_tdkit_report import __version__

    text = f"Generated by tdreport {__version__}"
    if ai:
        text += f" with AI narration, model: {model}" if model else " with AI narration"
    return text


def attribution(ai: bool = False, model: str | None = None) -> str:
    """Markdown footer line for reports (italic)."""
    return f"_{_credit(ai, model)}._"
