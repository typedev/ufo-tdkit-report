...
Everything here came out of using the tool for an afternoon rather than reading it.

### Added
- `-r` as a short alias for `--regenerate` — the flag typed most often while iterating on
  a draft.

### Changed
- **An outline redraw no longer prints a point count.** "~22 points moved" reads as
  precision and carries none: it does not distinguish a nudged terminal from a redrawn
  bowl, it is approximate anyway, and a page of them is tiring to read for a number nobody
  acts on. How many masters a change spans stays, because that answers a real question —
  one weight or all of them. `magnitude` is untouched: it still drives sort order and
  threshold folding, so the number lives where it does work and is gone where it did none.
  Both prompts gained a rule against the model supplying a count of its own, since a model
  trained on the old shape will otherwise write it straight back in.
- **A multi-master report closes with the masters it touched.** Every line says how many;
  none said which, and collecting that by hand off twenty lines is the work a report
  should have done for the reader. Rendered in all three shapes — report, release notes
  and the commit message — and omitted below two masters, where it would only restate what
  every line already says.

  Both change the bytes of every report containing outline changes across masters.
  Deliberate, and the owner's call.

### Fixed
- **An editable install no longer reports a stale version.** Editable metadata is frozen
  at install time while the code it runs moves on, so an install made at 0.4.1 kept
  stamping `0.4.1` into report footers and commit trailers generated by 0.5.x code — a
  false claim in the one field the versioning section calls a guarantee, and one that ends
  up in git history. `__version__` now reads `pyproject.toml` when the install is editable.
  Not a derived version: the same static number, no git and no dirty-tree suffix, so an
  editable and a real install at the same commit stamp identical bytes. Developing against
  an editable install is the normal way to work on this, and it should not cost the
  version's honesty.
- **Re-running on an unchanged repository no longer pays for a second narration.** Only a
  draft the owner had *edited* was kept; an untouched one was discarded and redrafted, so
  looking at the report twice meant two paid model calls for identical facts — and the
  sidecar had stored the facts digest that makes this decidable since the drafts feature
  shipped. A draft is now reused whenever it still fits: the facts are unchanged, and
  nothing was asked for that would produce different text (`--regenerate`, another
  `--ai-model`/`--ai-lang`/account, or a switch between `--no-ai` and narration each force
  a fresh one). Reported from real use. The decision lives in one place,
  `commit.reusable`, shared by the library and the CLI so the behaviour and the note
  explaining it cannot drift apart — and the CLI does say which draft you are looking at,
  because reusing silently would be the same invisible difference the loud fallback exists
  to prevent.
...
[Unreleased]: https://github.com/typedev/ufo-tdkit-report/compare/v0.5.4...HEAD
