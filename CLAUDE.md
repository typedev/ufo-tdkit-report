...
- **`rollup.py`** (`fold_facts`) is the compression core: it groups identical atoms across
  ...
  runs. An outline redraw does **not** print its point count: "~22 points moved" reads as
  measurement and separates nothing a reader acts on, and a page of them is tiring.
  `magnitude` still carries it — it drives sort order and threshold folding — so the
  number lives where it does work and is absent where it did none. Both prompts forbid the
  model supplying one of its own, since a model trained on the old shape will otherwise
  write it back in.
...
- **`render.py`** turns reports into text/markdown and owns the attribution footer.
  It closes a multi-master report with a **Masters touched** roll-call (`_render_masters`,
  and the plain-text equivalent in the commit message): each line says how *many* masters
  it spans, which answers "one weight or all of them", but not *which* — the question a
  reader is left holding. Omitted below two masters, because a section that is sometimes
  empty teaches people to skip it.
...
- **A draft is reused, not redrafted.** `commit.reusable` is the single judgement, shared
  by `inspect` and `cli` so the decision and the note explaining it cannot drift. An
  edited draft is the owner's; an *unedited* one is kept because regenerating it would
  produce the same text — staleness is measured on the facts, so "not stale" means the
  tree still describes exactly what the draft describes. Until 0.5.4 only the edited case
  was honoured, so looking at a report twice paid for two identical narrations. Forcing a
  fresh draft takes a deliberate act: `--regenerate`, a per-run AI override, or a change
  in whether narration is wanted at all. And say which draft is on screen — reusing
  silently is the same invisible difference the loud fallback exists to prevent.
...
it back from installed package metadata — except under an **editable** install, where the
metadata is frozen at install time while the code moves on, so `_editable_source_version`
reads `pyproject.toml` directly (`0.0.0+unknown` in an uninstalled source tree). That is
not a derived version: same static number, no git, no dirty-tree suffix, so an editable
and a real install at the same commit stamp identical bytes. It exists because an editable
install left at 0.4.1 spent a day stamping "0.4.1" into reports generated by 0.5.x code —
in the one field this section calls a guarantee. Developing against an editable install is
the normal way to work here; it must not cost the version's honesty.
